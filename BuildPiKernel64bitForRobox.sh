...
if [ ! -f "$SOURCE_IMGXZ" ]; then
  ...
  wget http://mirrors.tuna.tsinghua.edu.cn/ubuntu-cdimage/ubuntu/releases/"$SOURCE_RELEASE"/release/"$SOURCE_IMGXZ"
fi

# % Get Ubuntu source image 
if [ ! -f "$UBUNTU_IMGXZ" ]; then
  echo "Retrieving Ubuntu 19.10.1 source image ..."
  wget http://mirrors.tuna.tsinghua.edu.cn/ubuntu-cdimage/ubuntu/releases/19.10.1/release/ubuntu-19.10.1-preinstalled-server-arm64+raspi3.img.xz
fi
...
if [ ! -d "rpi-linux" ]; then
  ...
  # tbian: robox
  wget https://raw.githubusercontent.com/biantao712/Ubuntu-Server-raspi4-unofficial/robox/robox/exagear/euler-v4.19.25.patch
  patch -p1 < euler-v4.19.25.patch
  rm -rf euler-v4.19.25.patch
  ...
  # tbian: robox
  wget https://raw.githubusercontent.com/biantao712/Ubuntu-Server-raspi4-unofficial/robox/robox/exagear/conform_config_exagear.sh
  chmod +x conform_config_exagear.sh
  ./conform_config_exagear.sh
  rm -rf conform_config_exagear.sh
